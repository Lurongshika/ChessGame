...
func _run() -> void:
	...
	_check(scene.perks_black.size() == black_before, "审判逆位:敌方技能数量不变(失去一个重抽一个)")
	_check(not scene.perks_black.has("yuzhe") or not scene.perks_black.has("nvjisi"), "审判逆位:敌方失去一个技能")
